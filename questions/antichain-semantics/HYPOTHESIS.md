...
(mechanism, correlation-vs-redundancy analysis against `cd-index-type`,
TAGS/station/year breakdown, content sample). Headline: `antichain` group
membership is driven almost entirely by **in-degree** (effect −0.760, the
largest of any metric tested) and is essentially unrelated to a cable's
own out-degree (effect −0.025, negligible) — the opposite mechanism from
`cd-index-type`, which is out-degree-driven. The two still correlate
strongly (62% of `antichain=0` is `disruptive`) because both track the
same underlying archetype (a STATE circular with no predecessor of its
own, replied to by dozens of independent posts), not because they measure
the same thing — confirmed by trussness running in *opposite* directions
relative to the two attributes' correlated buckets, and by direct content
sampling (terminal field-post replies vs. widely-replied-to STATE
circulars).
...
- [`cd-index-semantics`](../cd-index-semantics/HYPOTHESIS.md) — found the
  ...
  reason (see `results/FINDINGS.md` §1-2). `cd-index-semantics`'s own predecessor-count
